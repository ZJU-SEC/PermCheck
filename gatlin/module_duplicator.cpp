...
ModuleDuplicator::ModuleDuplicator(Module &m, FunctionSet &keep,
                                   FunctionSet &remove) {
  ...
  for (auto p : vmap) {
    Value *v = const_cast<Value *>(static_cast<const Value *>(p.first));
    rvmap[p.second] = v;
  }
  ...
  for (auto f : to_erase) {
    ...
    FunctionCallee nf_callee = res_mod->getOrInsertFunction(
        n, f->getFunctionType(), f->getAttributes());
    auto *nf = dyn_cast<Function>(nf_callee.getCallee());
#else
    auto *nf = res_mod->getOrInsertFunction(n, f->getFunctionType(),
                                            f->getAttributes());
#endif
    /*
     * erase function
...
ModuleDuplicator::~ModuleDuplicator() { delete res_mod; }

Module &ModuleDuplicator::getResult() { return *res_mod; }

Value *ModuleDuplicator::map_to_duplicated(const Value *v) { return vmap[v]; }

Value *ModuleDuplicator::map_to_origin(const Value *v) { return rvmap[v]; }
