...
void gatlin::backward_slice_build_callgraph(InstructionList &callgraph,
  ...
  if (is_skip_function(f->getName().str())) {
    // should skip?
    ignored++;
  ...
  if ((chks != NULL) && chks->size()) {
    for (auto *chk : *chks) {
      if (dt.dominates(chk, I)) {
        if (knob_dump_good_path || I->getFunction()->getName().startswith("iscsi_if_rx")) {
...
good_out:
  fvisited[f] = RCHKED;
  if(yesno){
      errs() << "All Check Functions:";
      for (auto *chk : *chks) {
        errs() << get_callee_function_name(chk) << " \n ";
      }
  }
...
void gatlin::_check_critical_function_usage(Module *module, int tid,
  ...
  for (Function *func : critical_functions) {
    ...
    if (!crit_syms->use_builtin())             // means that not knob specified
      if (!crit_syms->exists(func->getName().str())) // means that symbol not matched
        continue;
    ...
    for (auto *U : func->users()) {
      ...
      get_callsite_inst(U, cil);
      for (auto cs : cil){
        if(func->getName().startswith("try_module_get") && cs->getFunction()->getName().startswith("iscsi_if_rx")){
          yesno = true;
          errs() << "cs user: iscsi_if_rx" << "\n";
        }
  ...
  for (auto cs : indirect_callsite_set) {
    ...
    for (auto func : *idcs2callee[cs]) {
      if (!crit_syms->use_builtin()) // means that not knob specified
        if (!crit_syms->exists(func->getName().str())) // means that symbol not
                                                 // matched
          continue;
      if (is_skip_function(func->getName().str()) ||
...
void gatlin::figure_out_gep_using_type_field(
  ...
  for (Module::iterator f = module.begin(), f_end = module.end(); f != f_end;
       ++f) {
    if (is_skip_function(dyn_cast<Function>(f)->getName().str()))
      continue;
    for (Function::iterator fi = f->begin(), fe = f->end(); fi != fe; ++fi) {
      BasicBlock *bb = dyn_cast<BasicBlock>(fi);
      for (BasicBlock::iterator ii = bb->begin(), ie = bb->end(); ii != ie;
...
void gatlin::crit_func_collect(CallInst *cs, FunctionSet &current_crit_funcs,
  ...
  if (Function *csf = get_callee_function_direct(cs)) {
    if (csf->isIntrinsic() || is_skip_function(csf->getName().str()) ||
        gating->is_gating_function(csf)) {
      if (is_skip_function(csf->getName().str()))
  ...
  } // else if (Value* csv = cs->getCalledOperand())
...
void gatlin::crit_vars_collect(Instruction *ii,
  ...
  if (gv && (!isa<Function>(gv)) && (!is_skip_var(gv->getName().str()))) {
    if (knob_gatlin_ccvv) {
...
void gatlin::process_cpgf(Module &module) {
  // collecting permission-check function
  // new added pass, put in gating
  // collectPermissions(module);
...
static RegisterPass<gatlin>
    XXX("gatlin-pass", "gatlin Pass (with getAnalysisUsage implemented)");
